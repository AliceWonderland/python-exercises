...
#convert decoder file to dict using comprehension #i didn't write this func
def Convert(lst):
...
def Decode(decoder):
    ...
    iterator = 1
    for line_num in range(1, decoder_len):
        if line_num != 1:
            encoded_number = iterator + line_num
            iterator = encoded_number
        else:
            encoded_number = line_num
